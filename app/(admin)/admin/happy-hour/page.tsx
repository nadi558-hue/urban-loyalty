...
export default async function AdminHappyHourPage() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 pt-8 pb-12" dir="rtl">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-bold" style={{ color: 'var(--urban-dark)' }}>
                    {it.title}
                    <span className="mr-2 text-xs font-bold px-2 py-0.5 rounded-full" style={{ color: '#b8860b', background: 'rgba(196,160,90,0.15)' }}>
                      +{it.bonus_coins} UC
                    </span>
                  </p>
              ...
              <div className="flex gap-2 mt-3">
                <form action={toggleActive} className="flex-1">
                  ...
                  <button type="submit" className="w-full py-2 rounded-xl text-xs font-bold" style={{ background: '#f3ede0', color: 'var(--urban-dark)' }}>
                    {it.active ? 'השהה' : 'הפעל'}
                  </button>
                </form>
                <form action={deletePromoted} className="flex-1">
                  <input type="hidden" name="id" value={it.id} />
                  <button type="submit" className="w-full py-2 rounded-xl text-xs font-bold" style={{ background: '#f3ede0', color: '#b04040' }}>
                    מחק
                  </button>
                </form>
              </div>
      ...
      <form action={addPromoted} className="bg-[#faf6ee] rounded-2xl border border-[#e8e0d0] p-4">
        ...
        <div className="space-y-2.5">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-500">
            בונוס UC
            <input name="bonus_coins" type="number" min={1} defaultValue={5} className="w-20 px-2 py-1.5 rounded-lg border border-[#e0d8c8] text-sm text-center outline-none" />
          </label>
